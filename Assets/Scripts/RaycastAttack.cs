...
public class RaycastAttack : NetworkBehaviour {
    // Assuming you have a reference to the "gameManager" GameObject
    GameObject gameManager;

    // Access the "Score" script component
    Score scoreScript;
            ...
            if (Runner.GetPhysicsScene().Raycast(ray.origin, ray.direction * shootDistance, out var hit)) {
                ...
                if (hitObject.TryGetComponent<Health>(out var health))
                {
                    if (hitObject == gameObject)
                    {
                        ...
                        // Handle the logic for hitting yourself (e.g., reduce your own health)
                    }
                    else
                    {
                        // You are hitting another object
